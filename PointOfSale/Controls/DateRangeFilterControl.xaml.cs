...
/// <summary>
/// Shared date-range filter: 4 quick-range chips (Day / Week / Month / Period)
/// plus a From / To / Apply group that is shown only when
/// <see cref="IsPeriodFilterVisible"/> is true.
///
/// Generic and technology-agnostic: it does NOT own a DataContext.
/// All properties are bound to the host page's ViewModel by the consumer
/// using a <c>RelativeSource AncestorType=UserControl</c> binding.
///
/// Consumers: HomeView, Transactions, Shifts.
/// </summary>
public partial class DateRangeFilterControl : UserControl
{
    ...
    // CurrentFilterMode, so a TwoWay binding would crash at attach with
    ...
    // ── Labels (localization pass-through) ────────────────────────────

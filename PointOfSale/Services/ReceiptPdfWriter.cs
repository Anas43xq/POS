...
public sealed class ReceiptPdfWriter : IReceiptFileWriter
{
    ...
    {
        // QuestPDF requires a one-time community-license opt-in
        // before any document is generated. We set it once per process.
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Ensure the parent directory exists so the write does not fail
        // with "directory not found" on a fresh machine.
        ...
        // QuestPDF's Document.Create is synchronous; we wrap the
        // generation in Task.Run so the calling ViewModel can await
        // a real Task and so cancellation is honoured between the
        // layout phases. For typical receipt sizes (<1 page) this is
        ...
        return Task.Run(() =>
        {
            ...
            var document = Document.Create(container =>
            {
                container.Page(page =>
                {
                    // Page size = paper width × auto height.
                    // No fixed page height so a long receipt can wrap
                    // naturally; QuestPDF will paginate as needed.
                    page.Size(new PageSize(
                        width: paperWidthMm > 0 ? paperWidthMm : 80,
                        height: 297,    // A4 height in mm; receipt is

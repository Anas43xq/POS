...
{
    /// <summary>
    /// Attached behavior that wires keyboard shortcuts declared in
    /// <c>shortcuts.json</c> to commands on the view's view-model.
    ///
    /// Usage in XAML:
    /// <code>
    /// <UserControl ...
    ///          b:ShortcutBindingsBehavior.Role="Manager" />
    /// </code>
    ///
    /// The behavior reads every key in the corresponding section of
    /// <c>shortcuts.json</c>, parses it, and attaches a
    /// <see cref="KeyBinding"/> to the view that invokes the matching
    /// <c>ICommand</c> property on the view's
    /// <c>DataContext</c> view-model (looked up by reflection).
    /// </summary>
    public static class ShortcutBindingsBehavior
    {
        // ── Attached property: Role ─────────────────────────────────
        ...
        // ── Attached property: Actions (optional subset) ──────────
        // Comma-separated list of action names. If unset, every
        // action in the role's section is wired. Set this to scope
        // the bindings (e.g. only "Refresh" on the Home view).
        ...
        private static void OnChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            // Always start from a clean slate so re-toggling the
            // property doesn't pile up duplicates.
            fe.InputBindings.Clear();

            var role = GetRole(fe);
            if (role == ShortcutRole.None) return;

            // Defer to Loaded so DataContext is set.
        ...
        private static void WireBindings(object? sender, RoutedEventArgs e)
        {
            ...
            fe.Loaded -= WireBindings; // one-shot
        ...
        // ── Command resolver ──────────────────────────────────────
        // Reflects the view-model DataContext and finds a public
        // ICommand property whose name matches the desired name.
        // Tries: exact, then "{Name}Command", then "ends with
        // {Name}Command" — so the action "Home" matches the VM
        // property "HomeCommand" or even "NavigateHomeCommand".
        ...
        // ── Action → (key, command) registry ──────────────────────
        // Hard-coded mapping from the JSON key names to the view-model
        // command property names. The JSON's key values are read at
        // runtime; this map only tells us "which key on the JSON
        // corresponds to which command on the VM".
    ...
    /// <summary>
    /// Which sub-section of <c>shortcuts.json</c> a view reads.
    /// </summary>

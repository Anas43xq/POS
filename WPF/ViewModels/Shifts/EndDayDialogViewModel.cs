    ...
    public class EndDayDialogViewModel : BaseViewModel
    {
        ...
        private async Task EndDayAsync()
        {
            try
            {
                ...
                if (_currentShift == null || _currentShift.ShiftId <= 0)
                {
                    ErrorMessage = _localizationService.GetString("Shift.NoActiveShiftFound");
                    return;
                }
                ...
                if (!decimal.TryParse(ClosingCash, out decimal closingCash))
                {
                    ErrorMessage = _localizationService.GetString("Shift.InvalidClosingCash");
                    return;
                }

                if (closingCash < 0)
                {
                    ErrorMessage = _localizationService.GetString("Shift.ClosingCashNegative");
                    return;
                }
                ...
                {
                    ErrorMessage = result.Error ?? _localizationService.GetString("Shift.CloseShiftFailed");
                }
            }
            catch (Exception ex)
            {
                ErrorMessage = _localizationService.GetString("Shift.GenericError", ex.Message);
            }
        }
    }
}
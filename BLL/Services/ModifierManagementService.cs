...
public class ModifierManagementService : IModifierManagementService
{
    ...
    {
        var validationError = ValidateGroup(dto);
    ...
    public async Task<Result<bool>> DeleteGroupAsync(int groupId)
    {
        try
        {
            ...
            // guard is the only thing standing between deleting the group and
            // silently removing every category's/product's assignment to it.
    ...
    {
        var validationError = ValidateOption(dto);
        if (validationError != null)
            return Result<int>.Failure(validationError);
    ...
    private static string? ValidateGroup(ModifierGroupWriteDto dto)
    {
        if (string.IsNullOrWhiteSpace(dto.Name))
            return "Group name is required.";

        if (dto.MinSelections < 0)
            return "Min selections must be a non-negative number.";

        if (dto.MaxSelections < 0)
            return "Max selections must be a non-negative number.";

        if (dto.MaxSelections < dto.MinSelections)
            return "Max selections must be >= Min selections.";

        if (dto.IsRequired && dto.MinSelections < 1)
            return "Required groups must have Min selections >= 1.";

        return null;
    }

    private static string? ValidateOption(ModifierOptionWriteDto dto)
    {
        if (string.IsNullOrWhiteSpace(dto.Name))
